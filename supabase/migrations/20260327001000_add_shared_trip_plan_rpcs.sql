...
begin
    ...
    if not public.trip_plan_actor_is_participant(trip_data) then
    ...
    if exists (
        ...
        where not public.trip_plan_row_targets_participant(target_user_id, trip_data)
    ) then
    ...
    select target_user_id, share_trip_plan.trip_id, share_trip_plan.trip_data
    from unnest(target_user_ids) as target_user_id
...
grant execute on function public.share_trip_plan(uuid[], uuid, jsonb) to authenticated;
grant execute on function public.delete_shared_trip_plan(uuid[], uuid, jsonb) to authenticated;
