...
async function translateWithFallback(source, target) {
  if (Date.now() >= googleRateLimitedUntil) {
    try {
      return await googleTranslate(source, target);
    } catch (error) {
      ...
      if (message.includes("TooManyRequestsError")) {
        ...
        console.warn(`google failed (${target}): ${message}`);
      }
    }
  }

  return bingTranslate(source, target);
}
...
async function main() {
  ...
  for (const [locale, target] of localeMap) {
    ...
    for (const group of chunk(missing, 4)) {
      const translated = await Promise.all(
        group.map(async ({ key, source }) => ({
          key,
          text: await translateWithFallback(source, target),
        }))
      );
